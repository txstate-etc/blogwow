 ...
 */
public class BlogWowDaoImpl extends HibernateCompleteGenericDao implements
    BlogWowDao {
  ...
  public List getLocationsForBlogsIds(Long[] blogIds) {
    String hql = "select distinct blog.location from BlogWowBlog blog where blog.id in "
        + arrayToInString(blogIds) + " order by blog.location";
   ...
   * (non-Javadoc)
   * 
   * @see org.sakaiproject.blogwow.dao.BlogWowDao#getBlogPermEntries(java.lang.Long[],
  ...
  public List getBlogPermEntries(Long[] blogIds, String userId,
    ...
    String hql = "from BlogWowEntry entry where entry.blog.id in (:blogIds) "
    ...
    if (readLocations != null && readLocations.length > 0) {
      hql += " or (entry.privacySetting = :privacyGroup and "
          + "entry.blog.location in (:readLocations))";
      params.put("privacyGroup", BlogConstants.PRIVACY_GROUP);
      params.put("readLocations", readLocations);
    }
    if (readAnyLocations != null && readAnyLocations.length > 0) {
      hql += " or (entry.privacySetting = :privacyGroupLeader and "
          + "entry.blog.location in (:readAnyLocations))";
